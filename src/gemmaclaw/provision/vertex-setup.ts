...
export async function interactiveVertexSetup(opts?: {
  ...
  if (useServiceAccount) {
    ...
    try {
      const saData = JSON.parse(fs.readFileSync(saKeyPath, "utf-8")) as { client_email?: string };
      const clientEmail = saData.client_email;
      if (!clientEmail) {
        throw new Error("Missing client_email in service account key file.");
      }
      accessToken =
        execFileSync(
          "gcloud",
