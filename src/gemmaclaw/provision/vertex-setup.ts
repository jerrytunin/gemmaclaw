...
export async function interactiveVertexSetup(opts?: {
  ...
  if (useServiceAccount) {
    ...
    try {
      if (!saKeyPath) {
        throw new Error("saKeyPath is undefined");
      }
      const saKeyData = JSON.parse(fs.readFileSync(saKeyPath, "utf-8"));
      const clientEmail = saKeyData.client_email;
      if (!clientEmail) {
        throw new Error("No client_email in service account key");
      }
